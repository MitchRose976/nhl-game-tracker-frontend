...
function Top10PowerplayPoints() {
  ...
  const getContent = async () => {
    top10PowerplayPoints.forEach((player) => {
      content.push(
        <tr key={player._id}>
          <td style={tdStyle}>
          ...
          {/* Player Points */}
          <td>{player.playerStats.stat.powerPlayPoints}</td>
        </tr>
      )
    })
  };
  ...
  return (
    <Accordion.Item eventKey="11">
      <Accordion.Header>Top 10 Powerplay Points</Accordion.Header>
      <Accordion.Body>
...
export default Top10PowerplayPoints;
